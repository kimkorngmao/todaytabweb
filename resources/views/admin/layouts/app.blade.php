...
<head>
    ...
    <title>@yield('title', $siteSettings->where('key', 'site-title')->first()->value ?? 'Today Tab')@hasSection('title') - {{$siteSettings->where('key', 'site-title')->first()->value ?? 'Today Tab'}} @endif</title>
    ...
    <aside id="sidebar" class="hidden md:block w-64 flex-shrink-0 h-screen bg-white fixed md:sticky top-0 left-0 flex flex-col justify-between border-r border-gray-100 z-40">
        <div class="text-gray-900 w-full">
            ...
            <ul class="mt-5 p-4 flex flex-col gap-2">
                ...
                @if (auth()->user()->hasPermission('admin.logins.index'))
                    <li>
                        <a href="{{ route('admin.logins.index') }}"
                            class="flex items-center gap-2 px-4 py-3 rounded-md hover:bg-gray-50 duration-200 {{ request()->routeIs('admin.logins.*') ? 'bg-blue-50 font-medium' : '' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 20 20"><path fill="currentColor" d="M9 6v5h.06l2.48 2.47l1.41-1.41L11 10.11V6z"/><path fill="currentColor" d="M10 1a9 9 0 0 0-7.85 13.35L.5 16H6v-5.5l-2.38 2.38A7 7 0 1 1 10 17v2a9 9 0 0 0 0-18z"/></svg>
                            <span>Logins</span>
                        </a>
                    </li>
                @endif

                @if (auth()->user()->hasPermission('admin.visitors.index'))
                    <li>
                        <a href="{{ route('admin.visitors.index') }}"
                            class="flex items-center gap-2 px-4 py-3 rounded-md hover:bg-gray-50 duration-200 {{ request()->routeIs('admin.visitors.*') ? 'bg-blue-50 font-medium' : '' }}">
